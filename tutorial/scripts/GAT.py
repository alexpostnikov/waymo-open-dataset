...
# create connectivity graph from from states
def create_connectivity_graph(states):
    """
    :param states: tensor of shape (bs, N, data_dim) where N is the number of nodes in the graph and data_dim is the
    dimensionality of the node features
    :return: connectivity_mask: tensor of shape (2, E) where E is the number of edges in the graph, and the first
    dimension is a tensor of shape (E,) with the source node indices, and the second dimension is a tensor of shape
    (E,) with the target node indices
    """
    bs = states.shape[0]
    num_peds_max = states.shape[1]
    num_connections_max = (num_peds_max * (num_peds_max - 1)) // 2
    # connectivity_masks int type
    connectivity_masks = torch.zeros(2, bs * num_connections_max, dtype=torch.int64)
    for batch_idx in range(states.shape[0]):
        # create shift for each batch
        shift = batch_idx * num_peds_max
        connectivity_mask = itertools.combinations(range(states.shape[1]), 2)
        # connectivity_mask to tensor
        connectivity_mask = torch.tensor(list(connectivity_mask), dtype=torch.int64) + shift
        # connectivity_mask shape num_connections, 2
        connectivity_mask = connectivity_mask.transpose(0, 1)
        # add to connectivity_mask
        connectivity_masks[:,
            batch_idx * (num_connections_max): (batch_idx + 1) * num_connections_max] = connectivity_mask
    return connectivity_masks


if __name__ == '__main__':
    # num_in_features, num_out_features, num_of_heads, concat = True, activation = nn.ELU(),
    # dropout_prob = 0.6, add_skip_connection = True, bias = True, log_attention_weights = False
    num_in_features = 256
    ...
    # init model2
    model2 = GATLayerImp3(num_in_features, num_out_features, num_of_heads, concat=True, activation=nn.ELU(),
                          dropout_prob=0.6, add_skip_connection=True, bias=True, log_attention_weights=False)
    # init data ( in_nodes_features and connectivity_mask )
    in_nodes_features = torch.randn(100, num_in_features)
    # connectivity_mask shape 2, 10000 type int64
    connectivity_mask = torch.randint(0, 100, (2, 10000), dtype=torch.int64)
    # run model2
    out_nodes_features, connectivity_mask = model2((in_nodes_features, connectivity_mask))
    print(out_nodes_features.shape)
    print(connectivity_mask.shape)

    # define num_of_layers, num_heads_per_layer, num_features_per_layer
    num_of_layers = 3
    num_heads_per_layer = [8, 8, 8]
    num_features_per_layer = [2*11+1, 2*11+1, 2*11+1, 2*11+1]
    # init GAT model
    model = GAT(num_of_layers, num_heads_per_layer, num_features_per_layer)
    # states is bs, num_agents, num_features
    states = torch.rand(16, 128, 2*11+1)
    # create connectivity graph
    connectivity_mask = create_connectivity_graph(states)


    # run model
    # states shape was bs, num_p, data
    # make it bs*num_p, data
    states_comb = states.view(states.shape[0] * states.shape[1], states.shape[2])
    out_nodes_features, connectivity_mask = model((states_comb, connectivity_mask))

    # out_nodes_features transform to bs, num_p, data
    out_nodes_features = out_nodes_features.view(states.shape[0], states.shape[1], out_nodes_features.shape[-1])
    print(out_nodes_features.shape)
    print(connectivity_mask.shape)
    # measure time cost of model and  create_connectivity_graph
    import time
    start = time.time()
    for i in range(10):
        out_nodes_features, connectivity_mask = model((states_comb, connectivity_mask))
    ...
    # move to cuda
    states_comb = states_comb.cuda()
    connectivity_mask = connectivity_mask.cuda()
    #model to cuda
    model = model.cuda()
    # measure time cost of model and  create_connectivity_graph
    start = time.time()
